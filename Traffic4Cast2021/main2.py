...
def modify_options(options, n_params):
    filename = '_'.join(
        [f"{item}" for item in (options.city_category or options.city, options.net_type, options.blk_type,
                                int(n_params))])
    options.filename = options.checkpoint.split(os.sep)[0] or filename
...
def get_trainer(options):  # gpus, max_epochs=20):
    ...
    if options.resume and options.checkpoint:
        resume_from_checkpoint = os.path.join(options.log_dir, options.checkpoint)
    #resume_from_checkpoint = None
...
def train(options):
    ...
    if options.checkpoint:
        model = Model.load_from_checkpoint(os.path.join(options.log_dir, options.checkpoint))
    else:
        model = Model(options)
    # print(model)
...
def add_main_args(parent_parser):
    ...
    parser.add_argument('--checkpoint', default='',  #'../version_0/checkpoints/last.ckpt',
                        type=str,
                        help='model checkpoint to work with [file/folder(to resume training or start fine-tuning)]')
