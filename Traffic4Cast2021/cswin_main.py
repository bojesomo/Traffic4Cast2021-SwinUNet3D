...
from Traffic4Cast2021.cswin_model_pl import Model, AllModel  # all_models
...
class DataModule(pl.LightningDataModule):
    ...
    def add_data_specific_args(parent_parser):
        ...
        parser.add_argument('--sampling-step', default=1,
...
def modify_options(options, n_params):
    ...
    filename = '_'.join(
        [f"{item}" for item in (options.city_category or options.city, options.model_name, options.backbone,
                                options.head, int(n_params))])
...
def get_trainer(options):  # gpus, max_epochs=20):
    ...
    trainer = pl.Trainer(gpus=options.gpus,
                         ...
                         plugins=DDPPlugin(num_nodes=options.nodes, find_unused_parameters=True),
...
def train(options):
    ...
    # # del model
    # x_all = torch.rand(2, options.in_channels, 495, 436)
    # # if options.blk_type.lower().endswith('3d'):
    # #     x_all = torch.rand(1, options.n_frame_in, options.in_channels, 495, 436)
    # _ = model_summary(model, x_all, print_summary=True, max_depth=2)
...
def add_main_args(parent_parser):
    ...
    parser.add_argument("-j", "--workers", type=int, required=False, default=6,
    ...
    parser.add_argument('--time-code', default='',
                    help='identifier for model if already exist')
    parser.add_argument('--use_checkpoint', type=bool, default=False, help='memory_efficient')
...
def get_time_code():
    ...
    if os.path.exists('t.npy'):
        ...
        if abs(int(''.join(time_before)) - int(''.join(time_now))) < 120:
...
def main():
    ...
    options.collapse_time = not options.backbone.lower().endswith('3d')
