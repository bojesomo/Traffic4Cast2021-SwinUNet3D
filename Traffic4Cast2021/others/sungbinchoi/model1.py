...
class DenseBlock(nn.Module):
    def __init__(self, in_channels, h_size, nb_layers):
        super().__init__()
        hidden_channels = in_channels
        self.layers = nn.ModuleList()
        for i in range(nb_layers):
            layer = ConcatOutput(Conv3x3ActNorm(hidden_channels, h_size))
...
class AvgDenseBlock(nn.Sequential):
    def __init__(self, in_channels, h_size, nb_layers):
        super().__init__()
        self.add_module('pool', nn.AvgPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True))
        self.add_module('dense', DenseBlock(in_channels, h_size, nb_layers))


class NetA(nn.Module):
    def __init__(self, input_channels, out_size):
        super().__init__()
        self.encode1 = DenseBlock(input_channels, 64, 2)
        self.encode2 = AvgDenseBlock(64, 96, 2)
        self.encode3 = AvgDenseBlock(96, 128, 2)
        self.encode4 = AvgDenseBlock(128, 128, 2)
        self.encode5 = AvgDenseBlock(128, 128, 2)
        self.encode6 = AvgDenseBlock(128, 128, 2)
        self.encode7 = AvgDenseBlock(128, 128, 2)
        self.encode8 = AvgDenseBlock(128, 128, 2)
        ...
        self.decode2 = UpscaleConcatConvActNorm(128, 96, 128)
        self.decode1 = UpscaleConcatConvActNorm(128, 64, 128)
